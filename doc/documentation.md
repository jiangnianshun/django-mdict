...
* 新标签页正查：在全文搜索页面和单词典查询页面选择文字，单击新窗口查询，如果新标签页正查打开，则新窗口打开查询页面进行查询，否则打开全文搜索或单词典查询页面进行查询。

* 精确计算高度：计算iframe中每一个元素的高度来获取总高度，展开速度会稍慢一些，目的是解决某些词典使用absolute定位导致高度获取不正确。
